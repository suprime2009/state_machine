...
object AppRunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dbName = system.settings.config.getString("database.db.properties.databaseName")
    val url = "jdbc:mysql://" +
    ...
    val flyway: Flyway = Flyway
      .configure()
      .dataSource(url + "mysql", user, pass)
      .initSql(s"CREATE DATABASE IF NOT EXISTS $dbName;")
      .schemas(dbName)
      .load()
